...
from .release import generate_release_dir
...
BACKGROUND_COMMANDS = ["daemon", "tray"]
UTILITY_COMMANDS = ["deps-check", "release-dir", "release-pack"]
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        nargs="?",
        default="run",
        choices=["run", "preview", "show", "gui", *BACKGROUND_COMMANDS, *AUTOSTART_COMMANDS, *UTILITY_COMMANDS],
        help="run=上报; preview/show=本地查看; gui=桌面版; daemon/tray=后台模式; autostart-*=开机自启管理; deps/release=工具",
    )
    ...
    p.add_argument("--all", dest="only_success", action="store_false", help="包含失败请求（默认仅成功）")
    p.add_argument("--no-verify-tls", dest="verify_tls", action="store_false", help="不校验 TLS 证书（自签名时用）")
    p.add_argument("--no-incremental", dest="incremental", action="store_false", help="忽略状态水位线（不影响 signature 去重）")
    p.add_argument("--dry-run", action="store_true", help="演练，不实际上报")
    p.add_argument("--start-hidden", action="store_true", help="GUI 启动时最小化/隐藏窗口")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "gui":
        ...
        return launch_gui(config_path=args.config, start_hidden=args.start_hidden)

    if args.command == "daemon":
        from .daemon import run_daemon
        return run_daemon(config_path=args.config)
    ...
    if args.command == "autostart-install":
        print(install_autostart(config_path=args.config, start_hidden=True))
        return 0
    if args.command == "autostart-uninstall":
        print(uninstall_autostart())
        return 0
    if args.command == "autostart-status":
        status = get_autostart_status()
        print(f"platform={status.platform} enabled={status.enabled} path={status.path}")
        return 0
    ...
    if args.command == "release-dir":
        outdir = generate_release_dir()
        print(f"release_dir={outdir}")
        return 0

    if args.command == "release-pack":
        outdir = generate_release_dir(pack=True)
        print(f"release_dir={outdir}")
        return 0
    ...
    if args.command in {"preview", "show"}:
        try:
            ...
            return 1
        print(f"聚合桶总数: {len(buckets)}（app_types={cfg.app_types}, only_success={cfg.only_success}）")
        ...
        for b in buckets[:limit]:
            print(f"  {b.date} {b.app_type:6} {b.model:26} req={b.request_count:4} in={b.input_tokens:>10} out={b.output_tokens:>8} cost=${b.total_cost_usd}")
