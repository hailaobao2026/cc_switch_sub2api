...
type ExternalUsageDailyRow struct {
	UserID              int64   `json:"user_id"`
...
func buildExternalUsageWhereClause(c *gin.Context) (string, []any, error) {
	...
	if value := strings.TrimSpace(c.Query("source")); value != "" {
		addCondition("source = $"+strconv.Itoa(len(args)+1), value)
	}
	if value := strings.TrimSpace(c.Query("app_type")); value != "" {
		addCondition("app_type = $"+strconv.Itoa(len(args)+1), value)
	}
	if value := strings.TrimSpace(c.Query("model")); value != "" {
		addCondition("model ILIKE $"+strconv.Itoa(len(args)+1), "%"+value+"%")
	}
	if value := strings.TrimSpace(c.Query("user")); value != "" {
		addCondition("(username ILIKE $"+strconv.Itoa(len(args)+1)+" OR email ILIKE $"+strconv.Itoa(len(args)+1)+")", "%"+value+"%")
	}
	if value := strings.TrimSpace(c.Query("start_date")); value != "" {
		if _, err := time.Parse("2006-01-02", value); err != nil {
			return "", nil, err
		}
		addCondition("usage_date >= $"+strconv.Itoa(len(args)+1), value)
	}
...
func (h *UsageHandler) ExternalDaily(c *gin.Context) {
	...
	var total int64
	if err := h.db.QueryRowContext(c.Request.Context(), "SELECT COUNT(*) FROM admin_external_usage_v WHERE "+whereClause, args...).Scan(&total); err != nil {
	...
	query := `SELECT user_id, COALESCE(username, ''), COALESCE(email, ''), source, usage_date, app_type, model, requested_model, request_count, success_count, input_tokens, output_tokens, cache_read_tokens, cache_creation_tokens, input_tokens + output_tokens + cache_read_tokens + cache_creation_tokens AS total_tokens, total_cost::float8, reported_at FROM admin_external_usage_v WHERE ` + whereClause + ` ORDER BY ` + orderColumn + ` ` + sortOrder + `, model ASC LIMIT $` + strconv.Itoa(limitParam) + ` OFFSET $` + strconv.Itoa(offsetParam)
	...
	items := make([]ExternalUsageDailyRow, 0, pageSize)
	for rows.Next() {
		var item ExternalUsageDailyRow
		var usageDate time.Time
		var reportedAt time.Time
		if err := rows.Scan(&item.UserID, &item.Username, &item.Email, &item.Source, &usageDate, &item.AppType, &item.Model, &item.RequestedModel, &item.RequestCount, &item.SuccessCount, &item.InputTokens, &item.OutputTokens, &item.CacheReadTokens, &item.CacheCreationTokens, &item.TotalTokens, &item.TotalCost, &reportedAt); err != nil {
			response.InternalError(c, err.Error())
			return
		}
		item.UsageDate = usageDate.Format("2006-01-02")
...
func (h *UsageHandler) ExternalStats(c *gin.Context) {
	...
	stats := ExternalUsageStatsResponse{
		Models:  make([]usagestats.ModelStat, 0),
		Apps:    make([]usagestats.EndpointStat, 0),
		Sources: make([]usagestats.EndpointStat, 0),
	}
...
SELECT
  ...
  COALESCE(SUM(total_cost::float8), 0),
  COUNT(*),
  COUNT(DISTINCT user_id)
FROM admin_external_usage_v
WHERE `
	if err := h.db.QueryRowContext(c.Request.Context(), summaryQuery+whereClause, args...).Scan(
		&stats.TotalRequests,
		&stats.TotalSuccessRequests,
	...
	stats.TotalActualCost = stats.TotalCost
	if stats.TotalRequests > 0 {
		stats.SuccessRate = float64(stats.TotalSuccessRequests) * 100 / float64(stats.TotalRequests)
	}
...
WHERE ` + whereClause + `
GROUP BY model
ORDER BY COALESCE(SUM(input_tokens + output_tokens + cache_read_tokens + cache_creation_tokens), 0) DESC, model ASC
LIMIT 12`
	modelRows, err := h.db.QueryContext(c.Request.Context(), modelsQuery, args...)
	if err != nil {
		response.InternalError(c, err.Error())
		return
	}
	...
	const endpointDimensionQueryTemplate = `
SELECT
  %s,
  COALESCE(SUM(request_count), 0),
  COALESCE(SUM(input_tokens + output_tokens + cache_read_tokens + cache_creation_tokens), 0),
...
WHERE ` + "%s" + `
GROUP BY %s
ORDER BY COALESCE(SUM(input_tokens + output_tokens + cache_read_tokens + cache_creation_tokens), 0) DESC, %s ASC
LIMIT 12`

	loadEndpointStats := func(column string) ([]usagestats.EndpointStat, error) {
		query := fmt.Sprintf(endpointDimensionQueryTemplate, column, whereClause, column, column)
		...
		items := make([]usagestats.EndpointStat, 0)
		for rows.Next() {
			var item usagestats.EndpointStat
			if err := rows.Scan(&item.Endpoint, &item.Requests, &item.TotalTokens, &item.Cost); err != nil {
				return nil, err
			}
			item.ActualCost = item.Cost
			items = append(items, item)
	...
	if stats.Apps, err = loadEndpointStats("app_type"); err != nil {
		response.InternalError(c, err.Error())
		return
	}
	if stats.Sources, err = loadEndpointStats("source"); err != nil {
		response.InternalError(c, err.Error())
		return
	}
...
ORDER BY usage_date ASC`
	...
	items := make([]ExternalUsageTrendPoint, 0)
	for rows.Next() {
		var item ExternalUsageTrendPoint
		var usageDate time.Time
		if err := rows.Scan(
			&usageDate,
			&item.Requests,
		...
		item.Date = usageDate.Format("2006-01-02")
		item.ActualCost = item.Cost
		items = append(items, item)
	}
	if err := rows.Err(); err != nil {
